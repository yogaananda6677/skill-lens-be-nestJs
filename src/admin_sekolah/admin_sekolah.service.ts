...
export class AdminSekolahService {
  constructor(
    ...
    private readonly dataSource: DataSource, // tambah
  ) {}
  ...
  async listSiswa(userId: number, query: any) {
    ...
    // tahunAjaran belum difilter karena kolom belum ada
  ...
  private validateSchoolDto(dto: CreateAdminSchoolDto) {
    ...
    if (noTelp) {
      ...
      if (!/^[0-9+]+$/.test(noTelp)) {
        throw new BadRequestException('Nomor telepon hanya boleh angka atau tanda +.');
      }
      if (digitOnly.length < 6 || digitOnly.length > 15) {
        throw new BadRequestException('Nomor telepon sekolah harus 6 sampai 15 digit.');
      }
    }
  }
  ...
  private validateTeacherDto(dto: CreateAdminTeacherDto) {
    ...
    if (!nama) throw new BadRequestException('Nama guru wajib diisi.');
    if (nama.length < 3) throw new BadRequestException('Nama guru minimal 3 karakter.');
    if (!/^[A-Za-zÀ-ÿ\s.'-]+$/.test(nama))
      throw new BadRequestException('Nama guru hanya boleh berisi huruf, spasi, titik, petik, atau tanda hubung.');
    if (!email) throw new BadRequestException('Email guru wajib diisi.');
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email))
      throw new BadRequestException('Format email guru tidak valid.');
    if (!username) throw new BadRequestException('Username guru wajib diisi.');
    if (username.length < 5) throw new BadRequestException('Username minimal 5 karakter.');
    if (!/^[a-z]/.test(username)) throw new BadRequestException('Username harus diawali huruf.');
    if (!/[0-9]/.test(username))
      throw new BadRequestException('Username guru wajib memiliki minimal 1 angka.');
    if (!/^[a-z0-9._]+$/.test(username))
      throw new BadRequestException('Username hanya boleh huruf kecil, angka, titik, dan underscore.');
    if (!nip) throw new BadRequestException('NIP/NUPTK wajib diisi.');
    if (nip.length < 5) throw new BadRequestException('NIP/NUPTK minimal 5 karakter.');
    if (!/^[0-9]+$/.test(nip)) throw new BadRequestException('NIP/NUPTK hanya boleh angka.');
    if (noHp) {
      const digitOnly = noHp.replace(/^\+/, '');
      if (!/^(\+62|62|08)[0-9]+$/.test(noHp))
        throw new BadRequestException('Nomor HP harus diawali 08, 62, atau +62.');
  ...
  async createOrUpdateSchool(userId: number, dto: CreateAdminSchoolDto) {
    ...
    if (currentSchool?.status_verifikasi === 'approved') {
      throw new ConflictException('Sekolah sudah diverifikasi dan tidak bisa diajukan ulang.');
    }
    if (currentSchool?.status_verifikasi === 'pending') {
      throw new ConflictException('Pengajuan sekolah masih menunggu verifikasi superadmin.');
    }

    const npsn = this.clean(dto.npsn);
    if (npsn) {
      const duplicateNpsn = await this.sekolahRepo.findOne({ where: { npsn } });
      if (duplicateNpsn) throw new ConflictException('NPSN sudah digunakan oleh sekolah lain.');
    }

    const jenisSekolah = this.clean(dto.jenis_sekolah).toUpperCase() as 'SMA' | 'SMK';
  ...
  async getApprovedSchoolOrFail(userId: number) {
    ...
    if (!sekolah) {
      throw new BadRequestException('Ajukan data sekolah terlebih dahulu sebelum menggunakan fitur ini.');
    }
  ...
  async createTeacher(userId: number, dto: CreateAdminTeacherDto) {
    ...
    if (existingUser) throw new ConflictException('Email atau username guru sudah digunakan.');

    const existingGuru = await this.guruRepo.findOne({ where: { nip } });
    if (existingGuru) throw new ConflictException('NIP/NUPTK sudah terdaftar.');
  ...
  async createJurusan(userId: number, body: any) {
    ...
    if (!namaJurusan) throw new BadRequestException('Nama jurusan wajib diisi.');
    if (namaJurusan.length < 2) throw new BadRequestException('Nama jurusan minimal 2 karakter.');
    if (namaJurusan.length > 80) throw new BadRequestException('Nama jurusan maksimal 80 karakter.');

    const existing = await this.jurusanRepo.findOne({
      where: { nama_jurusan: namaJurusan, id_sekolah: sekolah.id_sekolah },
    });
    if (existing) throw new ConflictException('Jurusan sudah ada di sekolah ini.');
  ...
  async getNilaiSiswa(siswaId: number, userId: number) {
    ...
    const nilaiRows = await this.dataSource
      ...
      .find({
        ...
        relations: ['kurikulum_mapel', 'kurikulum_mapel.mata_pelajaran'],
        order: { id_nilai: 'ASC' },
      });
    ...
    for (const row of nilaiRows) {
      const namaMapel = row.kurikulum_mapel?.mata_pelajaran?.nama_mapel?.trim();

      if (!namaMapel) continue;

      const idKurikulumMapel =
        row.id_kurikulum_mapel ?? row.kurikulum_mapel?.id_kurikulum_mapel;

      const key = namaMapel.toLowerCase();

      nilaiMap.set(key, {
        id_nilai: row.id_nilai,
        id_kurikulum_mapel: idKurikulumMapel,
        nama_mapel: namaMapel,
        nilai: row.nilai,
      });
    }

    const data = Array.from(nilaiMap.values()).sort((a, b) =>
