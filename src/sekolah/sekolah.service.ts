...
export class SekolahService {
  ...
  async findAll(): Promise<SchoolResponse[]> {
    const rows = await this.sekolahRepo.find({ order: { nama_sekolah: 'ASC' } });
    return rows.map((row) => this.toResponse(row));
  }

  async findApproved(): Promise<SchoolResponse[]> {
    const rows = await this.sekolahRepo.find({ where: { status_verifikasi: 'approved' }, order: { nama_sekolah: 'ASC' } });
    return rows.map((row) => this.toResponse(row));
  }

  async create(data: any) {
    const namaSekolah = String(data?.nama_sekolah ?? data?.namaSekolah ?? '').trim();
    const jenisSekolah = String(data?.jenis_sekolah ?? data?.jenisSekolah ?? '').trim().toUpperCase();
    ...
    const row = await this.sekolahRepo.save(
      this.sekolahRepo.create({
        ...
        no_hp_sekolah: data?.no_hp_sekolah ? String(data.no_hp_sekolah).trim() : null,
        jenis_sekolah: jenisSekolah as 'SMA' | 'SMK',
        status_verifikasi: data?.status_verifikasi === 'approved' ? 'approved' : 'pending',
      }),
    );
  ...
  private toResponse(row: Sekolah): SchoolResponse {
    return {
      ...
      accreditation: row.status_verifikasi === 'approved' ? 'Terverifikasi' : 'Menunggu verifikasi',
      address: row.alamat ?? '-',
      totalStudents: 0,
