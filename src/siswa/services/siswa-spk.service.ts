...
export class SiswaSpkService {
  ...
  async prosesSpk(userId: number, body: any) {
    ...
    const payload = {
      ...
      /**
       * Semua tag di bawah ini adalah mapped_key dari master_tags.
       * Engine tidak lagi melakukan alias/normalisasi semantik.
       */
    ...
    return {
      message: recommendations.length
        ? 'Rekomendasi berhasil diproses dan disimpan sebagai histori baru.'
  ...
  private normalizeRecommendations(result: any) {
    ...
    return rows.map((item: any, index: number) => ({
      ...
      /**
       * Jangan fallback ke alternatif_id/id.
